...
class Panel:
    ...
    def _monitor_tts(self):
        import queue as q
        cola_dir = "c:/Proyectos/Lector/tts_queue/"
        os.makedirs(cola_dir, exist_ok=True)
        cola_legacy = "c:/Proyectos/Lector/tts_cola.txt"
        contador = [0]

        while True:
            try:
                # Lee archivo legacy (tts_cola.txt)
                if os.path.exists(cola_legacy):
                    with open(cola_legacy, "r", encoding="utf-8") as f:
                        contenido = f.read()
                    os.remove(cola_legacy)
